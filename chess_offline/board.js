...
function getKingMoves(col, row, color) {
    ...
    if (!king.hasMoved && king.color == color) {
        if (canCastle(color, 'king')) color == 'w' ? moves.push('71') : moves.push('78');
        if (canCastle(color, 'queen')) color == 'w' ? moves.push('31') : moves.push('38');
    }
    return moves;
}
...
function canCastle(color, side) {
    ...
    let ksq = document.querySelectorAll('[class*="is-checked"]');
    if (ksq.length > 0 && ksq[0].classList.contains(`square-5${row}`)) return false;
